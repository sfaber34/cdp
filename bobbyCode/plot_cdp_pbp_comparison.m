...
filename = '1Hz.csv';
data = csvread(filename, 126, 0);

CDP_bins = [3.000000,4.000000,5.000000,6.000000,7.000000,8.000000,9.000000,10.000000,11.000000,12.000000,13.000000,14.000000,16.000000,18.000000,20.000000,22.000000,24.000000,26.000000,28.000000,30.000000,32.000000,34.000000,36.000000,38.000000,40.000000,42.000000,44.000000,46.000000,48.000000,50.000000]
CDP_conc = data(:,15:44);
...
threshold_high = 1000;
 ...
 figure
 PBP_n = PBP_conc('1Hz_PBP.csv', time);
 conc = sum(CDP_conc,2);
 scatter(sum(CDP_conc,2), sum(CDP_conc,2)-PBP_n', 49, 'r', 'filled');
 hold on;
 plot([0 1.1*max(conc)], [0 0], 'k', 'LineWidth', 2);
 xlim([0 1.1*max(conc)]);
 ylim([-100 1000]);
 xlabel('Total PBP concentration for time period');
 ylabel('Total binned counts-total PBP counts');
